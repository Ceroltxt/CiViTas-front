...
const teamColorMap: Record<string, { bg: string, text: string }> = {
  'bg-violet-600': { bg: 'bg-violet-100 dark:bg-violet-500/20', text: 'text-violet-700 dark:text-violet-300' },
  'bg-emerald-500': { bg: 'bg-emerald-100 dark:bg-emerald-500/20', text: 'text-emerald-700 dark:text-emerald-300' },
  'bg-orange-500': { bg: 'bg-orange-100 dark:bg-orange-500/20', text: 'text-orange-700 dark:text-orange-300' },
  'bg-blue-500': { bg: 'bg-blue-100 dark:bg-blue-500/20', text: 'text-blue-700 dark:text-blue-300' },
  'bg-amber-500': { bg: 'bg-amber-100 dark:bg-amber-500/20', text: 'text-amber-700 dark:text-amber-300' },
  'bg-pink-500': { bg: 'bg-pink-100 dark:bg-pink-500/20', text: 'text-pink-700 dark:text-pink-300' },
  'bg-cyan-500': { bg: 'bg-cyan-100 dark:bg-cyan-500/20', text: 'text-cyan-700 dark:text-cyan-300' },
}
...
<template>
 <div v-if="project" class="mx-auto max-w-7xl space-y-5 p-4 sm:p-6">
  ...
  <!-- Tarefas do projeto (escondidas por padrão) -->
  <section class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
   <div class="flex flex-col gap-3 border-b border-slate-100 p-4 lg:flex-row lg:items-center lg:justify-between">
    <div class="flex items-center gap-2">
     <h2 class="font-semibold text-slate-800 dark:text-slate-100">Tarefas do projeto</h2>
     <button
      type="button"
      class="grid size-6 place-items-center rounded-md text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
      :title="showTasks ? 'Esconder tarefas' : 'Mostrar tarefas'"
      @click="showTasks = !showTasks"
     >
      <UIcon
       :name="showTasks ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'"
       class="size-4"
      />
     </button>
    </div>
    <div v-if="showTasks" class="flex gap-1 overflow-x-auto"><button v-for="view in views" :key="view.id" type="button" class="relative flex shrink-0 items-center gap-1.5 px-3 py-2 text-sm font-semibold after:absolute after:bottom-0 after:inset-x-3 after:h-0.5" :class="activeView === view.id ? 'text-orange-600 after:bg-orange-500' : 'text-slate-400 hover:text-slate-600'" @click="activeView = view.id"><UIcon :name="view.icon" class="size-4" />{{ view.label }}</button></div>
   </div>
   <div v-if="showTasks" class="space-y-3 p-4"><div class="flex flex-wrap gap-2"><UInput v-model="search" icon="i-heroicons-magnifying-glass" placeholder="Buscar tarefas" class="w-full sm:w-56" /><UPopover><UButton color="neutral" variant="outline" size="sm" icon="i-heroicons-list-bullet" :label="statuses.length ? String(statuses.length) + ' status' : 'Status: todos'" /><template #content><div class="w-56 p-2"><button v-for="status in useStatusOptions()" :key="status.value" class="flex w-full items-center justify-between rounded-lg px-2 py-2 text-sm hover:bg-slate-50" @click="toggleStatus(status.value)"><span>{{ status.label }}</span><UIcon v-if="statuses.includes(status.value)" name="i-heroicons-check" class="size-4 text-orange-500" /></button></div></template></UPopover><UPopover><UButton color="neutral" variant="outline" size="sm" icon="i-heroicons-flag" :label="priorities.length ? String(priorities.length) + ' prioridades' : 'Prioridade: todas'" /><template #content><div class="w-52 p-2"><button v-for="priority in usePriorityOptions()" :key="priority.value" class="flex w-full items-center justify-between rounded-lg px-2 py-2 text-sm hover:bg-slate-50" @click="togglePriority(priority.value)"><span>{{ priority.label }}</span><UIcon v-if="priorities.includes(priority.value)" name="i-heroicons-check" class="size-4 text-orange-500" /></button></div></template></UPopover></div>
    <TarefasTaskTable v-if="activeView === 'lista' && filtered.length" :tasks="filtered" team-mode :teams="project.teams" />
