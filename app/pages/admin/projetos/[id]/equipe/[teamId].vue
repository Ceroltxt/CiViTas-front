...
onMounted(async () => {
  allProjects.value = await fetchAllProjectsFromSupabase()
})
const project = computed(() => allProjects.value.find((p: any) => String(p.id) === projectId))
...
<template>
  <div class="mx-auto max-w-7xl space-y-5 p-4 sm:p-6">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-slate-400">
      <NuxtLink to="/admin/projetos" class="hover:text-slate-600">Projetos</NuxtLink>
      <UIcon name="i-heroicons-chevron-right" class="size-3" />
      <NuxtLink :to="`/admin/projetos/${projectId}`" class="hover:text-slate-600">
      ...
      <section class="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white px-5 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-xs text-slate-400">
            ...
            Projeto:
            <NuxtLink :to="`/admin/projetos/${projectId}`" class="text-orange-500 hover:underline">
            ...
            <table class="w-full text-left">
              ...
              <tbody>
                <tr
                  ...
                  @click="navigateTo(`/admin/projetos/${projectId}`)"
                >
      ...
      <template v-if="activeTab === 'membros'">
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div class="flex items-center justify-between mb-4">
            ...
            <UButton size="sm" icon="i-heroicons-user-plus" class="bg-violet-600 hover:bg-violet-700 text-white" @click="openAddMember">
              Adicionar Membro
            </UButton>
          </div>
    ...
    <UModal v-model:open="addMemberOpen" :ui="{ content: 'max-w-md' }">
      ...
      <template #body>
        <div class="space-y-3">
          ...
          <UInput v-model="addMemberSearch" icon="i-heroicons-magnifying-glass" placeholder="Buscar por nome..." class="w-full" />
