import { a as getNavigationForRole, r as resolveAppRole } from './navigation-CS4vmRJG.mjs';
...
const v = {
  ...
  number() {
    return {
      parse(value, path = "valor") {
        return typeof value === "number" && !Number.isNaN(value) ? value : fail(path, "número", value);
      }
    };
  },
  boolean() {
  ...
  enum(...allowed) {
    return {
      parse(value, path = "valor") {
        return allowed.includes(value) ? value : fail(path, `um de [${allowed.join(", ")}]`, value);
      }
    };
  },
  ...
  optional(schema) {
    return {
      parse(value, path = "valor") {
        return value === void 0 || value === null ? void 0 : schema.parse(value, path);
      }
    };
  },
  array(schema) {
    ...
    return {
      parse(value, path = "valor") {
        ...
        for (const key in shape) {
          ...
          result[key] = shape[key].parse(record[key], childPath);
        }
        return result;
      }
    };
...
const userSummarySchema = v.object({
  id: v.string(),
  name: v.string(),
  role: v.optional(v.string()),
  avatar: v.optional(v.string())
});
const currentProjectSchema = v.object({
  name: v.string(),
  deadline: v.string(),
  progress: v.number()
});
const prioritySchema = v.enum("critica", "alta", "media", "baixa");
const statusSchema = v.enum(
  "planejado",
  "em-andamento",
  "em-revisao",
  "bloqueado",
...
const projectDetailSchema = v.object({
  name: v.string(),
  deadline: v.string(),
  progress: v.number(),
...
const dashboardMetricSchema = v.object({
  id: v.string(),
  label: v.string(),
  value: v.string()
});
...
const rankingEntrySchema = v.object({
  position: v.number(),
...
const teamSummarySchema = v.object({
  id: v.string(),
  name: v.string(),
  initial: v.string(),
  color: v.string(),
...
const teamProjectSchema = v.object({
  name: v.string(),
  description: v.string(),
  status: v.string(),
  startDate: v.string(),
...
const teamRecentTaskSchema = v.object({
  id: v.string(),
  title: v.string(),
  status: statusSchema,
  createdBy: v.string(),
...
const teamDetailSchema = v.object({
  id: v.string(),
  name: v.string(),
  initial: v.string(),
  color: v.string(),
...
const calendarEventSchema = v.object({
  id: v.string(),
  title: v.string(),
  startDay: v.number(),
  length: v.number(),
...
const timelineTaskSchema = v.object({
  id: v.string(),
  title: v.string(),
...
const timelineGroupSchema = v.object({
  title: v.optional(v.string()),
  tasks: v.array(timelineTaskSchema)
});
const tasksSchema = v.array(taskSchema);
const notificationsSchema = v.array(notificationSchema);
const auditLogsSchema = v.array(auditLogSchema);
const calendarEventsSchema = v.array(calendarEventSchema);
const timelineGroupsSchema = v.array(timelineGroupSchema);
const teamDetailsSchema = v.array(teamDetailSchema);
const PEOPLE = {
...
getNavigationForRole(resolveAppRole(mockCurrentUser.role));
...
function mockAuditLogs(count = 10) {
...
export { teamDetailsSchema as A, teamSummarySchema as B, timelineGroupsSchema as C, userSummarySchema as D, v as E, agendaItemSchema as a, auditLogsSchema as b, calendarEventsSchema as c, currentProjectSchema as d, dashboardMetricSchema as e, mockAuditLogs as f, mockBoardTasks as g, mockCalendarEvents as h, mockCurrentProject as i, mockCurrentUser as j, mockCurrentUserRank as k, mockDashboardMetrics as l, mockAgenda as m, mockNotifications as n, mockProjectDetail as o, mockProjects as p, mockRanking as q, mockTasks as r, mockTeamDetails as s, mockTeams as t, mockTimeline as u, notificationsSchema as v, projectDetailSchema as w, projectProgressSchema as x, rankingEntrySchema as y, tasksSchema as z };
//# sourceMappingURL=index-D-j7ajRT.mjs.map
